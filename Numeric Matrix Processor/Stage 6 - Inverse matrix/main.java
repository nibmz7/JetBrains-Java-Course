...
public class Main {
    ...
    static double[][] inverseMatrix(double[][] matrix) throws Exception {
        ...
        double[][] innerMatrix = new double[rows][columns];

        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < columns; j++) {
                double[][] subMatrix = getSubMatrix(matrix, i, j);
                double cofactor = Math.pow(-1, i + j) * calculateDeterminant(subMatrix);
                innerMatrix[i][j] = cofactor;
            }
        }

        return multiplyMatrixByConstant(transposeMainDiagonal(innerMatrix), 1 / determinant);

    }

    static double[][] getSubMatrix(double[][] matrix, int excludeRow, int excludeColumn) {
        int rows = matrix.length;
        int columns = matrix[0].length;
